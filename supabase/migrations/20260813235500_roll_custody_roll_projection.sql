-- Cube D — make confirmed current custody an invariant of the Roll row itself.
...
alter table public.rolls
  add column custodian_party_id uuid
...
set
  custodian_party_id = rc.custodian_party_id,
  custody_confirmed_at = rc.confirmed_at
from public.roll_custody_current rc
where rc.roll_id = r.id;
...
create index rolls_custodian_party_idx
  on public.rolls (custodian_party_id, id);

comment on column public.rolls.custodian_party_id is
  'Operational Party holding confirmed physical custody of the Roll. Never an Auth user/profile id.';
comment on column public.rolls.custody_confirmed_at is
  'Timestamp when the current confirmed custody began. Pending Transfers do not change this value.';
