-- Cube M — Warranty Activation, increment 2 compatibility hardening
-- Preserve Cube K's issue_pending Recovery preflight while adding the later
-- effective-Warranty boundary required by Cube M.
...
comment on function public.resolve_opened_roll_recovery_candidate(text) is
  'Cube J/K Recovery preflight extended by Cube M: effective Warranty blocks Recovery while Cube K issue_pending and existing reservation/destination states remain intact.';
