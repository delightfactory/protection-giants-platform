...
export type CompleteResolutionByAdminRecoveryResult =
...
async function authorizeAdminRecoveryEvidence(resolutionId: string): Promise<boolean> {
  ...
  const { data, error } = await supabase.rpc("get_admin_warranty_claim_resolution_detail", {
  ...
  const detail = data[0];
  if (
    detail.resolution_id !== resolutionId
    || detail.resolution_status !== "assigned"
    || detail.claim_status !== "approved"
    || detail.claim_closed_at !== null
    || !detail.performing_center_party_id
    || !["service_reinstall", "replacement_roll_reinstall"].includes(detail.remedy_kind ?? "")
  ) {
    return false;
  }
  ...
  return detail.performing_center_status === "suspended"
    || (detail.performing_center_status === "active" && activeOperatorCount === 0);
}
...
export async function uploadAdminRecoveryCompletionEvidence(
  resolutionId: string,
  slot: number,
  file: File,
): Promise<RecoveryEvidenceUploadResult> {
  ...
  if (!(await authorizeAdminRecoveryEvidence(resolutionId))) {
    return { ok: false, code: "PG_CLAIM_RESOLUTION_ADMIN_RECOVERY_NOT_ALLOWED" };
  }
  ...
  const evidence: RecoveryEvidenceReference = {
    storagePath,
  ...
  if (existing) {
    ...
    if (metadata?.mimeType === detectedMime && metadata.sizeBytes === bytes.length) {
      return (await authorizeAdminRecoveryEvidence(resolutionId))
        ? { ok: true, evidence }
        : { ok: false, code: "PG_CLAIM_RESOLUTION_ADMIN_RECOVERY_NOT_ALLOWED" };
    }
  ...
  if (!uploadError) {
    if (await authorizeAdminRecoveryEvidence(resolutionId)) return { ok: true, evidence };
...
export async function removeAdminRecoveryCompletionEvidence(
  resolutionId: string,
  storagePath: string,
...
export async function completeWarrantyClaimResolutionByAdminRecovery(
  input: CompleteResolutionByAdminRecoveryInput,
): Promise<CompleteResolutionByAdminRecoveryResult> {
  if (!UUID_PATTERN.test(input.requestId) || !UUID_PATTERN.test(input.resolutionId)) {
    return { ok: false, code: "PG_CLAIM_RESOLUTION_ADMIN_RECOVERY_REQUEST_INVALID" };
  }
  ...
  if (recoveryReason.length < 5 || recoveryReason.length > 500) {
    return { ok: false, code: "PG_CLAIM_RESOLUTION_ADMIN_RECOVERY_REASON_INVALID" };
  }
  ...
  if (error) {
    return {
      ok: false,
      code: EXPOSED_RECOVERY_ERRORS.has(error.message)
        ? error.message
        : "PG_CLAIM_RESOLUTION_ADMIN_RECOVERY_FAILED",
    };
  }
  if (typeof data !== "string" || data !== input.resolutionId) {
    return { ok: false, code: "PG_CLAIM_RESOLUTION_ADMIN_RECOVERY_FAILED" };
  }
