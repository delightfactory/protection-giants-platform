...
export type CompletionEvidenceReference = { storagePath: string; mimeType: CompletionEvidenceMime; sizeBytes: number; slot: number };
...
export type CompleteAssignedResolutionResult = { ok: true; resolutionId: string } | { ok: false; code: string };
...
async function authorizeAssignedResolution(resolutionId: string): Promise<boolean> {
  ...
  const { data, error } = await supabase.rpc("get_center_warranty_claim_resolution_task", { p_resolution_id: resolutionId });
...
export async function uploadClaimResolutionCompletionEvidence(resolutionId: string, slot: number, file: File): Promise<CompletionEvidenceUploadResult> {
  ...
  if (!(await authorizeAssignedResolution(resolutionId))) return { ok: false, code: "PG_CLAIM_RESOLUTION_NOT_ASSIGNED_CENTER" };
  ...
  const evidence: CompletionEvidenceReference = { storagePath, mimeType: detectedMime, sizeBytes: bytes.length, slot };
  const objectName = storagePath.slice(storagePath.lastIndexOf("/") + 1);
  const supabase = await createSupabaseServerClient();
  const { error: stageError } = await supabase.rpc("register_warranty_claim_resolution_completion_evidence_stage", {
  ...
  if (stageError) {
    return {
      ok: false,
      code: stageError.message === "PG_CLAIM_RESOLUTION_NOT_ASSIGNED_CENTER"
        ? stageError.message
...
export async function removeClaimResolutionCompletionEvidence(resolutionId: string, storagePath: string): Promise<{ ok: boolean; code?: string }> {
...
export async function completeAssignedWarrantyClaimResolution(input: CompleteAssignedResolutionInput): Promise<CompleteAssignedResolutionResult> {
  if (!UUID_PATTERN.test(input.requestId) || !UUID_PATTERN.test(input.resolutionId)) return { ok: false, code: "PG_CLAIM_RESOLUTION_COMPLETE_REQUEST_INVALID" };
  ...
  if (error) return { ok: false, code: EXPOSED_COMPLETE_ERRORS.has(error.message) ? error.message : "PG_CLAIM_RESOLUTION_COMPLETE_FAILED" };
  if (typeof data !== "string" || data !== input.resolutionId) return { ok: false, code: "PG_CLAIM_RESOLUTION_COMPLETE_FAILED" };
