...
function querySql(sql) {
  return execFileSync("docker", ["exec", "-i", dbContainerName(), "psql", "-At", "-F", "|", "-v", "ON_ERROR_STOP=1", "-U", "postgres", "-d", "postgres", "-c", sql], { encoding: "utf8" }).trim();
}
...
function eventId(claimId, kind, extra = "") {
  const id = querySql(`
    select event.id from public.warranty_claim_events event
    where event.claim_id = ${sqlUuid(claimId)} and event.event_kind = '${kind}' ${extra}
...
function assertNotificationShape({ eventIdValue, eventType, expectedCount, attention, pushEligible, actionPath, recipientRole, claimNumber }) {
  const pathPredicate = actionPath === null
    ? "notification.action_path is null"
    : `notification.action_path = '${actionPath}'`;
  const rows = querySql(`
    select concat_ws('|',
      count(*),
      ...
      count(*) filter (where ${pathPredicate}),
...
const companyPartyId = querySql(`select id from public.operational_parties where party_type = 'company';`);
...
`);
assert(companyPartyId && centerAPartyId && centerBPartyId, "Company and Center A/B operational parties are required.");
const activeAdminProfiles = Number(querySql(`select count(*) from private.notification_party_profile_ids(${sqlUuid(companyPartyId)});`));
const activeCenterAProfiles = Number(querySql(`select count(*) from private.notification_party_profile_ids(${sqlUuid(centerAPartyId)});`));
const activeCenterBProfiles = Number(querySql(`select count(*) from private.notification_party_profile_ids(${sqlUuid(centerBPartyId)});`));
assert(activeAdminProfiles >= 1 && activeCenterAProfiles >= 1 && activeCenterBProfiles >= 1,
  "Active Company/Admin and Center A/B Profiles are required.");
...
const claimA = querySql(`
  ...
  where exists (
    ...
    where event.claim_id = claim.id and event.event_kind = 'approval_cancelled_before_execution'
  ) limit 1;
`).split("|");
...
const pendingReopenEventId = eventId(claimAId, "decision_reopened_for_correction", "and event.event_data ->> 'resumed_status' = 'awaiting_inspection'");
...
// Cube Q must preserve the frozen Cube P submitted notification contract.
...
`)) === 0, "review_started must remain notification-silent.");

assertNotificationShape({
  eventIdValue: inspectionRequestedEventId, eventType: "claim.inspection_requested",
  expectedCount: activeCenterAProfiles, attention: "action_required", pushEligible: true,
  actionPath: "/operations/claim-inspections", recipientRole: "center", claimNumber: claimANumber,
});
assertNotificationShape({
  eventIdValue: pendingCancelledEventId, eventType: "claim.inspection_cancelled",
  expectedCount: activeCenterAProfiles, attention: "info", pushEligible: false,
  actionPath: "/operations/claim-inspections", recipientRole: "center", claimNumber: claimANumber,
});
assert(notificationCount(pendingCancelledEventId, "claim.cancelled") === Math.max(0, activeAdminProfiles - 1),
  "Pending-inspection cancellation must exclude the acting Admin from internal self-success visibility.");
assertNotificationShape({
  eventIdValue: pendingReopenEventId, eventType: "claim.inspection_resumed",
  expectedCount: activeCenterAProfiles, attention: "action_required", pushEligible: true,
  actionPath: "/operations/claim-inspections", recipientRole: "center", claimNumber: claimANumber,
});
assert(notificationCount(pendingReopenEventId, "claim.decision_reopened_for_correction") === Math.max(0, activeAdminProfiles - 1),
  "PD-078 internal correction visibility must exclude the acting Admin.");
assertNotificationShape({
  eventIdValue: reassignedEventId, eventType: "claim.inspection_reassigned",
  expectedCount: activeCenterBProfiles, attention: "action_required", pushEligible: true,
  actionPath: "/operations/claim-inspections", recipientRole: "center", claimNumber: claimANumber,
});
assert(Number(querySql(`
  select count(*) from public.notifications notification
...
`)) === 0, "Inspection reassignment must notify the new Center, not the old Center.");
assertNotificationShape({
  eventIdValue: inspectionSubmittedEventId, eventType: "claim.inspection_submitted",
  expectedCount: activeAdminProfiles, attention: "action_required", pushEligible: true,
  actionPath: `/operations/claims/${claimAId}/review`, recipientRole: "admin", claimNumber: claimANumber,
});
...
assert(Number(querySql(`
  ...
  where notification.source_domain = 'warranty_claim'
    and (
      (coalesce(warranty.customer_phone, '') <> '' and position(warranty.customer_phone in notification.body) > 0)
      or (coalesce(warranty.customer_name, '') <> '' and position(warranty.customer_name in notification.body) > 0)
      or (coalesce(warranty.vehicle_vin, '') <> '' and position(warranty.vehicle_vin in notification.body) > 0)
    );
`)) === 0, "Claim notification bodies must not leak customer phone/name/VIN PII.");

// Add a second Admin only after the inherited single-Admin scenarios so internal
// decision/correction visibility can be proven positively without changing Q fixtures.
...
const createdSecondAdmin = await request("/auth/v1/admin/users", {
  ...
  body: {
    ...
    password,
    email_confirm: true,
    app_metadata: { pg_provisioning: { version: "operational-v1", role: "admin" } },
    user_metadata: { display_name: "Cube Q Notification Admin" },
  },
});
assert(createdSecondAdmin.response.ok && /^[0-9a-f-]{36}$/i.test(createdSecondAdmin.body?.id ?? ""),
  `Could not create second Admin fixture: ${createdSecondAdmin.response.status} ${JSON.stringify(createdSecondAdmin.body)}`);
const secondAdminId = createdSecondAdmin.body.id;
const secondAdminToken = await signIn(secondAdminEmail);
assert(Number(querySql(`select count(*) from private.notification_party_profile_ids(${sqlUuid(companyPartyId)});`)) === activeAdminProfiles + 1,
  "Second Admin must become an active Company notification recipient.");
...
const claimB = querySql(`
  ...
  where claim.id <> ${sqlUuid(claimAId)}
    ...
    and not exists (select 1 from public.warranty_claim_resolutions resolution where resolution.claim_id = claim.id)
    and not exists (
      select 1 from public.warranty_claims later_claim
      where later_claim.warranty_id = claim.warranty_id and later_claim.id <> claim.id
        and (later_claim.submitted_at > claim.submitted_at
          or (later_claim.submitted_at = claim.submitted_at and later_claim.id > claim.id))
    )
  order by claim.submitted_at desc, claim.id desc limit 1;
`).split("|");
...
assert(secondReopen.response.ok && typeof secondReopen.body === "string",
  `Could not create correction notification: ${secondReopen.response.status} ${JSON.stringify(secondReopen.body)}`);
const secondReopenEventId = secondReopen.body;
assertNotificationShape({
  eventIdValue: secondReopenEventId, eventType: "claim.decision_reopened_for_correction",
  expectedCount: 1, attention: "action_required", pushEligible: false,
  actionPath: `/operations/claims/${claimBId}/review`, recipientRole: "admin", claimNumber: claimBNumber,
});
assert(querySql(`
...
assert(finalReject.response.ok && typeof finalReject.body === "string",
  `Could not create final-decision notification: ${finalReject.response.status} ${JSON.stringify(finalReject.body)}`);
const finalRejectEventId = finalReject.body;
assertNotificationShape({
  eventIdValue: finalRejectEventId, eventType: "claim.rejected",
  expectedCount: 1, attention: "info", pushEligible: false,
  actionPath: `/operations/claims/${claimBId}`, recipientRole: "admin", claimNumber: claimBNumber,
});
assert(querySql(`
  select recipient_profile_id from public.notifications
  where source_domain = 'warranty_claim'
...
assert(querySql(`
  select count(*) from public.notification_push_deliveries delivery
...
`) === "0", "Internal decision/correction visibility must remain Inbox-only.");
...
assert(secondAdminInbox.body.some((item) =>
  ...
  "Other active Admin must see Claim correction/final-decision visibility in the existing Cube L Inbox.");
