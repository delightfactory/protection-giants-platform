...
function serial(lotSequence: number, rollIndex: number) {
  return `PG-R-20260825-00000001-${String(lotSequence).padStart(2, "0")}-${String(rollIndex).padStart(4, "0")}`;
}

function roll(idSuffix: number, lotId: string, lotSequence: number, rollIndex: number) {
  return {
...
function identitiesFor(plan: ReturnType<typeof buildFixture>) {
  const map = new Map<string, { rollId: string; publicCode: string }>();
  let index = 1;
