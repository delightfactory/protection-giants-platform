...
assert(querySql(`select has_function_privilege('authenticated', '${signature}', 'EXECUTE');`) === "t",
  "Authenticated users must reach the R assignment RPC; role enforcement happens inside it.");
...
const allocationCountBefore = querySql(`select count(*) from public.warranty_claim_resolution_roll_allocations;`);
assert(allocationCountBefore === "0",
  "Initial assignment verifier expects allocation persistence to be empty before its operation.");
...
const historicalQ = querySql(`
  ...
  where claim.status = 'cancelled' and claim.closed_at is not null
    and resolution.status = 'authorized' and warranty.record_state = 'issued'
  order by claim.updated_at desc, claim.id desc limit 1;
`).split("|");
...
await expectRpcError("assign_warranty_claim_resolution", {
  p_action_request_id: randomUUID(), p_resolution_id: historicalResolutionId,
  p_remedy_kind: "service_reinstall", p_performing_center_party_id: centerAPartyId,
...
await expectRpcError("assign_warranty_claim_resolution", {
  p_action_request_id: randomUUID(), p_resolution_id: resolutionId,
  p_remedy_kind: "unsupported_remedy", p_performing_center_party_id: centerAPartyId,
}, adminToken, "PG_CLAIM_RESOLUTION_REMEDY_INVALID");
await expectRpcError("assign_warranty_claim_resolution", {
  p_action_request_id: randomUUID(), p_resolution_id: resolutionId,
  p_remedy_kind: "service_reinstall", p_performing_center_party_id: randomUUID(),
}, adminToken, "PG_CLAIM_CENTER_INACTIVE");
const deniedCenter = await userRpc("assign_warranty_claim_resolution", {
  p_action_request_id: randomUUID(), p_resolution_id: resolutionId,
  p_remedy_kind: "service_reinstall", p_performing_center_party_id: centerAPartyId,
}, centerAToken);
assert(!deniedCenter.response.ok, "Center Profile must not perform Admin-only initial assignment.");
...
const assigned = await userRpc("assign_warranty_claim_resolution", {
  p_action_request_id: assignmentRequestId, p_resolution_id: resolutionId,
  p_remedy_kind: "service_reinstall", p_performing_center_party_id: centerAPartyId,
}, adminToken);
...
`).split("|");
assert(eventRow.length === 9 && eventRow[1] === resolutionId && eventRow[2] === "resolution_assigned"
  && eventRow[3] === adminProfileId && eventRow[4] === "admin" && eventRow[5] === "t"
  && eventRow[6] === claimId && eventRow[7] === "service_reinstall" && eventRow[8] === centerAPartyId,
  `Resolution assignment event drift: ${eventRow}`);
const assignmentEventId = eventRow[0];

const expectedCenterRecipients = querySql(`select count(*) from private.notification_party_profile_ids(${sqlUuid(centerAPartyId)});`);
...
assert(retry.response.ok && retry.body === resolutionId, "Exact assignment retry must return the same Resolution id.");
assert(querySql(`select count(*) from public.warranty_claim_resolution_events where action_request_id = ${sqlUuid(assignmentRequestId)};`) === "1",
  "Assignment retry must not duplicate the immutable event.");
assert(querySql(`
  select count(*) from public.notifications notification
  where notification.source_domain = 'warranty_claim_resolution'
    and notification.source_event_key = ${sqlText(`warranty_claim_resolution_events:${assignmentEventId}`)}
...
await expectRpcError("assign_warranty_claim_resolution", {
  p_action_request_id: assignmentRequestId, p_resolution_id: resolutionId,
  p_remedy_kind: "replacement_roll_reinstall", p_performing_center_party_id: centerAPartyId,
...
await expectRpcError("cancel_warranty_claim", {
  p_action_request_id: randomUUID(), p_claim_id: claimId,
...
assert(querySql(`
  ...
  join public.warranty_claims claim on claim.id = resolution.claim_id
  where resolution.id = ${sqlUuid(resolutionId)};
`) === "assigned|approved|t", "Q rejection must leave the R-assigned state unchanged.");
assert(querySql(`select count(*) from public.warranty_claim_resolution_roll_allocations;`) === allocationCountBefore,
  "Initial assignment must not create a Roll allocation as a side effect.");
