...
function none(result, label) {
  ...
  if (data.length !== 0) {
    throw new Error(`${label} unexpectedly returned data: ${JSON.stringify(data)}`);
  }
}
...
const companyDirect = await createCenter(adminToken, {
  code: "APP-C-COMP",
  name: "Approval Company Direct",
  country_code: "EG",
  city: "Tanta",
}, "Create Company-direct Center");
const suspendedA = await createCenter(adminToken, {
  code: "APP-C-SUSP",
  name: "Approval Suspended A",
  country_code: "EG",
  city: "Mansoura",
  country_agent_id: agentA.id,
}, "Create suspend-test Center");

for (const center of [noLocationA, agentDirectA, dealerChildA, dealerChildB, companyDirect, suspendedA]) {
...
mustFail(await rest("rpc/approve_center_network", adminToken, {
  method: "POST",
  body: { p_center_id: noLocationA.id },
...
mustFail(await rest("rpc/approve_center_network", adminToken, {
  method: "POST",
  body: { p_center_id: suspendedA.id },
}), "Admin approves suspended Center");

const approveDirectA = one(await rest("rpc/approve_center_network", agentAToken, {
  method: "POST",
  body: { p_center_id: agentDirectA.id },
}), "Agent A approves direct Center");
...
const approveChildA = one(await rest("rpc/approve_center_network", agentAToken, {
  method: "POST",
  body: { p_center_id: dealerChildA.id },
}), "Agent A approves Dealer-child Center");
if (!approveChildA.changed || approveChildA.approval_status !== "approved") {
  throw new Error(`Dealer-child approval contract mismatch: ${JSON.stringify(approveChildA)}`);
}

mustFail(await rest("rpc/approve_center_network", agentAToken, {
  method: "POST",
  body: { p_center_id: dealerChildB.id },
}), "Agent A approves Agent B Center");
mustFail(await rest("rpc/approve_center_network", agentAToken, {
  method: "POST",
  body: { p_center_id: companyDirect.id },
}), "Agent A approves Company-direct Center");

const approveChildB = one(await rest("rpc/approve_center_network", agentBToken, {
  method: "POST",
  body: { p_center_id: dealerChildB.id },
}), "Agent B approves own Dealer-child Center");
if (!approveChildB.changed || approveChildB.approval_status !== "approved") throw new Error("Agent B approval failed contract.");

mustFail(await rest("rpc/approve_center_network", dealerAToken, {
  method: "POST",
  body: { p_center_id: dealerChildA.id },
}), "Dealer approves Center");
mustFail(await rest("rpc/approve_center_network", centerAToken, {
  method: "POST",
  body: { p_center_id: dealerChildA.id },
}), "Center approves itself");

const approveCompany = one(await rest("rpc/approve_center_network", adminToken, {
  method: "POST",
  body: { p_center_id: companyDirect.id },
}), "Admin approves Company-direct Center");
...
const repeatApprove = one(await rest("rpc/approve_center_network", agentAToken, {
  method: "POST",
  body: { p_center_id: agentDirectA.id },
...
const directAEventsAfterRepeat = await approvalEvents(adminToken, agentDirectA.id);
if (directAEventsAfterRepeat.length !== directAEventsBeforeRepeat.length) throw new Error("Repeat approve appended a duplicate event.");
...
const directAfterRevoke = one(await rest(`installation_centers?id=eq.${agentDirectA.id}&select=id,status,approval_status,approved_at,approved_by_profile_id`, adminToken), "Admin reads revoked Center");
if (directAfterRevoke.status !== "active" || directAfterRevoke.approval_status !== "unapproved") {
  throw new Error("Revocation changed operational status or failed to reset approval.");
}
...
if ((await approvalEvents(adminToken, agentDirectA.id)).length !== eventsBeforeRepeatRevoke.length) throw new Error("Repeat revoke appended a duplicate event.");
...
if (allEventsForAdmin.length < 5) throw new Error(`Admin approval history is unexpectedly sparse: ${JSON.stringify(allEventsForAdmin)}`);
...
if (!agentAVisibleEvents.some((event) => event.installation_center_id === agentDirectA.id) || !agentAVisibleEvents.some((event) => event.installation_center_id === dealerChildA.id)) {
...
if (agentAVisibleEvents.some((event) => event.installation_center_id === dealerChildB.id || event.installation_center_id === companyDirect.id)) {
  throw new Error(`Agent A can see approval history outside own network: ${JSON.stringify(agentAVisibleEvents)}`);
}
...
}), "Admin bypasses approval RPC through generic Data API update");
mustFail(await rest(`installation_centers?id=eq.${dealerChildA.id}`, agentAToken, {
  method: "PATCH",
  prefer: true,
  body: { approval_status: "unapproved", approved_at: null, approved_by_profile_id: null },
}), "Agent bypasses approval RPC through generic Data API update");
mustFail(await rest("center_network_approval_events", adminToken, {
  method: "POST",
  prefer: true,
  body: {
    installation_center_id: dealerChildA.id,
    action: "revoked",
...
const childAEventBeforeLocation = await approvalEvents(adminToken, dealerChildA.id);
const locationChange = one(await rest("rpc/update_own_center_location", centerAToken, {
  method: "POST",
  body: { p_latitude: 30.0133, p_longitude: 31.2091, p_accuracy_m: 12 },
...
const childAAfterLocation = one(await rest(`installation_centers?id=eq.${dealerChildA.id}&select=id,approval_status,approved_at,approved_by_profile_id,latitude,longitude`, adminToken), "Admin reads Center after location change");
...
const childAEventAfterLocation = await approvalEvents(adminToken, dealerChildA.id);
if (childAEventAfterLocation.length !== childAEventBeforeLocation.length + 1) throw new Error("Location change did not append exactly one approval event.");
const invalidationEvent = childAEventAfterLocation.at(-1);
if (invalidationEvent.action !== "location_changed" || invalidationEvent.actor_profile_id !== centerAUser.id) {
  throw new Error(`Location invalidation audit mismatch: ${JSON.stringify(invalidationEvent)}`);
}

one(await rest("rpc/approve_center_network", adminToken, {
  method: "POST",
  body: { p_center_id: dealerChildA.id },
}), "Admin re-approves Center after location review");
...
mustFail(await rest("rpc/update_own_center_location", centerAToken, {
  method: "POST",
  body: { p_latitude: 30.02, p_longitude: 31.21, p_accuracy_m: 50.01 },
}), "Rejected location update while approved");
const childAAfterFailedLocation = one(await rest(`installation_centers?id=eq.${dealerChildA.id}&select=id,approval_status,approved_at,approved_by_profile_id`, adminToken), "Admin reads approval after failed location update");
if (childAAfterFailedLocation.approval_status !== "approved") throw new Error("Failed location update invalidated approval.");
if ((await approvalEvents(adminToken, dealerChildA.id)).length !== eventsBeforeFailedLocation.length) throw new Error("Failed location update appended approval history.");

one(await rest("rpc/admin_update_center_location", adminToken, {
  method: "POST",
  body: { p_center_id: dealerChildA.id, p_latitude: 30.0135, p_longitude: 31.2093 },
}), "Admin corrects approved Center location");
const childAAfterAdminCorrection = one(await rest(`installation_centers?id=eq.${dealerChildA.id}&select=id,approval_status`, adminToken), "Read approval after Admin location correction");
if (childAAfterAdminCorrection.approval_status !== "unapproved") throw new Error("Admin location correction did not invalidate approval.");
...
const immutableEvent = finalChildAEvents[0];
mustFail(await rest(`center_network_approval_events?id=eq.${immutableEvent.id}`, adminToken, {
  method: "PATCH",
  prefer: true,
  body: { action: "revoked" },
...
mustFail(await rest("rpc/approve_center_network", agentAToken, {
  method: "POST",
  body: { p_center_id: agentDirectA.id },
}), "Suspended Country Agent approves Center");
one(await rest(`country_agents?id=eq.${agentA.id}&select=id,status`, adminToken, {
  method: "PATCH",
  prefer: true,
  body: { status: "active" },
}), "Admin reactivates Agent A entity");
...
console.log("Center Network Approval database/RLS/audit verification passed.");
